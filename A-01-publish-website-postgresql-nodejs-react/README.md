# Project-A-01 : Web Page Application (Postgresql-Nodejs-React) deployed on EC2's with Ansible and Docker
...
- Codes written by developers should be pulled from frankyildiz Repo into the control node and sent them to the EC2's from here with Ansible.
...
```text
A-01:ansible_proj (folder)
|
|----Readme.md               # Given to the DevOps (Definition of the project)
|----todo-app-perm (folder)  # Given to the DevOps (Nodejs and React files)
|       1.server (folder) ---> Nodejs folders and files
|       2.client (folder) ---> React folders and files
|       3.database (folder)--> init.sql file 
|----developer_notes.txt     # Given to the DevOps (txt file)
|----Ansible-Playbook        # This will be created by DevOps
```
